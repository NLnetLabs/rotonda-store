...
use super::{cht::PrefixSet, map_type::MapType};
...
> {
    pub(crate) prefixes:
        &'a Cht<PrefixSet<PK, M, MT>, ROOT_SIZE, STRIDES_PER_BUCKET>,
    pub(crate) bmin: Option<&'a RoaringBitmap>,
    pub(crate) cur_len: u8,
    pub(crate) cur_bucket: &'a PrefixSet<PK, M, MT>,
    ...
    pub(crate) parents: [Option<(&'a PrefixSet<PK, M, MT>, usize)>; 8],
...
impl<
        ...
        const ROOT_SIZE: usize,
        const STRIDES_PER_BUCKET: usize,
    > Iterator for PrefixIter<'a, PK, M, MT, ROOT_SIZE, STRIDES_PER_BUCKET>
{
    type Item = (PK, Vec<Record<MT::Key, M>>);
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
            println!("PK::max_len() {}", PK::max_len());
            if self.cur_len > (PK::max_len() * 8) as u8 {
            ...
            if self.cursor >= 2_usize.pow(STRIDE_SIZE as u32) {
                if self.cur_level == 0 {
                    ...
                    // let's continue, get the prefixes for the next length
                    self.cur_bucket =
                        self.prefixes.root_for_len(self.cur_len);
                } else {
            ...
            if let Some(s_pfx) = self.cur_bucket.0.get(self.cursor) {
                // DEPTH FIRST ITERATION
                match s_pfx.next_bucket.0._is_null() {
                    false => {
                        ...
                        self.cur_bucket = &s_pfx.next_bucket;
