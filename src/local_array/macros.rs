                ...
                f: &|search_level: &SearchLevel<AF, $stride>,
                    ...
                    guard| {
                        ...
                        let index = $id.get_id().0.dangerously_truncate_to_usize()
                            >> (AF::BITS - NodeBuckets::<AF>::len_to_store_bits($id.get_id().1,level).unwrap());
                        ...
                        match unsafe { this_node.assume_init_mut() } {
                            ...
                            StoredNode::NodeWithRef((node_id, node, node_set)) => {
                                ...
                                level += 1;
                                match NodeBuckets::<AF>::len_to_store_bits($id.get_id().1, level) {
        ...
        $(
            SearchLevel {
                f: &|search_level: &SearchLevel<AF, $stride>,
                    ...
                    println!("level {}", level);
                    println!("bits_division {}", NodeBuckets::<AF>::len_to_store_bits($id.get_id().1,level).unwrap());
                    let guard = &epoch::pin();
                        ...
                        StoredNode::Empty => {
                            ...
                            std::mem::swap(
                                ...
                                &mut MaybeUninit::new(StoredNode::NodeWithRef((
                                    $id,
                                    new_node,
                                    NodeSet::init((1 << NodeBuckets::<AF>::len_to_store_bits($id.get_id().1, level + 1).unwrap()) as usize),
                                ))),
                            );
                            // ABA Baby!
