    ...
    > StarCastRib<M, MT, C, KEY_SIZE_IPV4, KEY_SIZE_IPV6>
{
    ...
    pub fn contains(&'a self, prefix: &Prefix, mui: Option<MT::Key>) -> bool {
        match prefix.addr() {
    ...
    pub fn less_specifics_iter_from(
        ...
        include_withdrawn: bool,
        guard: &'a Guard,
    ) -> impl Iterator<Item = FatalResult<PrefixRecord<MT::Key, M>>> + 'a
    {
    ...
    pub fn persist_prefixes_iter(
        &'a self,
    ) -> impl Iterator<Item = Result<PrefixRecord<MT::Key, M>, FatalError>> + 'a
    {
        self.v4
    ...
    pub fn mui_is_withdrawn_v4(&self, mui: MT::Key) -> bool {
        let guard = &epoch::pin();
    ...
    pub fn mui_is_withdrawn_v6(&self, mui: MT::Key) -> bool {
        let guard = &epoch::pin();
