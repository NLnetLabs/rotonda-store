...
mod test {
    ...
    fn test_more_specifics_without_less_specifics() -> Result<(), Box<dyn Error>> {
        let mut tree_bitmap = MultiThreadedStore::<PrefixAs>::new(vec![8], vec![8]);
        let pfxs = vec![
        ...
        for spfx in &[
            ...
            for i in spfx.2.iter() {
                ...
                let result_pfx = more_specifics.iter().find(|pfx| pfx.prefix == pfxs[*i]);
    ...
    fn test_more_specifics_with_less_specifics() -> Result<(), Box<dyn Error>> {
        ...
        for spfx in &[
            ...
            for i in spfx.2.iter() {
                ...
                let result_pfx = more_specifics.iter().find(|pfx| pfx.prefix == pfxs[*i].unwrap());
                assert!(result_pfx.is_some());
            }
            println!("-----------");
        }
