...
use rotonda_store::common::PrefixAs;
    ...
    fn test_insert_extremes_ipv4() -> Result<(), Box<dyn std::error::Error>> {
        let trie = &mut MultiThreadedStore::<NoMeta>::new(vec![3, 3, 3, 3, 3, 3, 3, 3, 4, 4], vec![8]);
        let min_pfx = Prefix::new_relaxed(std::net::Ipv4Addr::new(0, 0, 0, 0).into(), 1).unwrap();

        trie.insert(&min_pfx, NoMeta::Empty)?;
        let expect_pfx = Prefix::new_relaxed(std::net::Ipv4Addr::new(0, 0, 0, 0).into(), 1);
        ...
        let max_pfx = Prefix::new_relaxed(std::net::Ipv4Addr::new(255, 255, 255, 255).into(), 32);
        trie.insert(&max_pfx?, NoMeta::Empty)?;
        let expect_pfx = Prefix::new_relaxed(std::net::Ipv4Addr::new(255, 255, 255, 255).into(), 32);
        let res = trie.match_prefix(
            &expect_pfx?,
            &MatchOptions {
    ...
    fn test_tree_ipv4() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let pfxs = vec![
            ...
            Prefix::new_relaxed(std::net::Ipv4Addr::new(193, 0, 14, 0).into(), 23),
            Prefix::new_relaxed(std::net::Ipv4Addr::new(193, 0, 14, 0).into(), 24),
            Prefix::new_relaxed(std::net::Ipv4Addr::new(193, 0, 15, 0).into(), 24),
            Prefix::new_relaxed(std::net::Ipv4Addr::new(193, 0, 10, 10).into(), 32),
            Prefix::new_relaxed(0b0011_0000_0000_0000_0000_0000_0000_0000_u32.into_ipaddr(), 4),
        ...
        let res = tree_bitmap.match_prefix(
            &Prefix::new_relaxed(std::net::Ipv4Addr::new(192, 0, 1, 0).into(), 24)?,
            &MatchOptions {
                match_type: MatchType::LongestMatch,
                include_less_specifics: true,
                include_more_specifics: false,
        ...
        assert_eq!(
            ...
            Prefix::new_relaxed(std::net::Ipv4Addr::new(192, 0, 0, 0).into(), 4)?
        );
        Ok(())
    }
